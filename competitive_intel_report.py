...
def generate_comparison_rows(competitors):
    ...
    for comp in competitors:
        ...
        rows.append('        <tr class="' + row_class + '">\n'
...
def generate_gap_analysis(competitors):
    ...
    for comp in others:
        ...
        for k in cozzian['metrics']:
            ...
            if diff > 1.0:
                ...
                cozzian_adv.append(label + ' (+' + str(round(diff, 1)) + ')')

        adv_html = ''
        if cozzian_adv:
            for a in cozzian_adv:
                adv_html += '<li>' + a + '</li>\n'
        else:
            adv_html = '<li>Comparable -- no decisive advantage</li>\n'

        comp_adv_html = ''
        for s in comp['strengths']:
            comp_adv_html += '<li>' + s + '</li>\n'

        insight = _generate_insight(cozzian, comp)
    ...
    <div class="gap-card">
        ...
        <p class="gap-insight"><strong>Insight:</strong> ''' + insight + '''</p>
    </div>''')
    return '\n'.join(sections)
...
def _generate_insight(cozzian, competitor):
    ...
    if 'Lonza' in name:
        ...
        return ("Cozzian utterly dominates in regulatory support (+7.0), formulation speed (+3.5), "
                ...
                "Cozzian's compliant, quality-assured, sustainable process is a premium differentiator "
...
def main():
    ...
    print("Company: " + DATASET['company'])
    print("Competitors analyzed: " + str(len(DATASET['competitors'])))
    ...
    print("   File size: " + f"{file_size:,} bytes ({file_size/1024:.1f} KB)")
